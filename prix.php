...
<body>
	...
	<p><code class="copyright-text">La liste des marchandises disponible</code></p>
...
$sql="SELECT id_marchandise,description,unit_price FROM marchandise";
$result=$conn->query($sql);
	if($result->num_rows > 0){
		echo "<table><thead><tr><th>ID marchandise</th><th>Description</th><th>Prix</th></tr></thead>";
		echo "<tbody>";
		while($row=$result->fetch_assoc()){
			echo "<tr>";
			echo "<td>".$row["id_marchandise"]."</td><td>".$row["description"]."</td><td>".number_format((float)$row["unit_price"]/100.0, 2)."$</td>";
			echo "</tr>";
		}
		echo "</tbody>";
		echo "</table>";
		...
		echo "<h2>aucun marchandise n'est enregistré dans la base de données</h2>";
	}
    $conn->close();
?>
</div>
